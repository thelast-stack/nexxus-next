...
def make_flow(decision_status="approved"):
    ...
    proposal = ResolutionProposal(
        "prop-1", event.event_id, "Veículo indisponível durante frete", {"investigation": investigation.request_id},
        ({"type": "tow_vehicle", "vehicle": event.entity}, {"type": "authorize_repair", "vehicle": event.entity}),
...
def test_mismatched_decision_cannot_execute():
    _, _, proposal, decision, request = make_flow()
    forged_request = ExecutionRequest(request.request_id, proposal.proposal_id, "different-decision", request.requested_by)
