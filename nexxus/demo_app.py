...
class DemoState:
    ...
    def decide(self, status: str) -> None:
        if status not in {"approved", "rejected"} or not self.proposal:
            raise ValueError("a proposal and valid human decision are required")
    ...
    def snapshot(self) -> dict[str, object]:
        event = self.event
        return {"company": {"transportes": "ocorrência activa" if event else "operacional", "manutencao": "a analisar avaria" if event else "operacional", "financeiro": "impacto pendente" if event else "operacional", "pending_operations": 1 if event else 0}, "event": {"event_id": event.event_id, "vehicle": event.entity, "operation": event.facts["operation_id"]} if event else None, "messages": self.messages, "proposal": self.proposal.__dict__ if self.proposal else None, "decision": self.decision.__dict__ if self.decision else None, "request": self.request.__dict__ if self.request else None, "result": self.result.__dict__ if self.result else None, "master_interpretation": self.master_interpretation}
...
HTML = '''<!doctype html><html lang="pt"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NEXXUS NEXT</title><style>body{font-family:system-ui;margin:0;background:#f4f5f7;color:#17202a}header{padding:24px 32px;background:#111827;color:white}main{max-width:1100px;margin:24px auto;padding:0 20px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.card{background:white;border:1px solid #ddd;border-radius:12px;padding:18px;margin-bottom:16px}.dot{font-size:22px}button{padding:10px 14px;border:1px solid #bbb;border-radius:8px;background:white;cursor:pointer;margin:4px}button.primary{background:#111827;color:white}.msg{padding:9px;border-left:3px solid #aaa;margin:6px 0}.proposal{white-space:pre-wrap;background:#f8fafc;padding:12px;border-radius:8px}.status{font-weight:700;text-transform:uppercase}.small{color:#667085;font-size:13px}@media(max-width:800px){.grid{grid-template-columns:1fr}}</style></head><body><header><strong>NEXXUS NEXT</strong><div>Director Console · protótipo demonstrável</div></header><main><div class="card"><h2>Estado da empresa</h2><div class="grid" id="company"></div></div><div class="card"><h2>Operação</h2><div id="event">Nenhuma ocorrência activa.</div><button class="primary" onclick="post('/api/breakdown')">Simular avaria do veículo</button></div><div class="card"><h2>IA Mestre</h2><div id="messages"></div><div id="proposal"></div><div id="interpretation"></div></div><div class="card"><h2>Decisão do Director</h2><button onclick="post('/api/decision?status=approved')">Aprovar</button><button onclick="post('/api/decision?status=rejected')">Rejeitar</button><div id="decision"></div></div><div class="card"><h2>Execução experimental</h2><button onclick="post('/api/execute')">Executar com sucesso</button><button onclick="post('/api/fail')">Simular falha</button><div id="result"></div></div></main><script>async function post(u){const r=await fetch(u,{method:'POST'});const d=await r.json();if(!r.ok)alert(d.error);render(d)}async function load(){render(await (await fetch('/api/state')).json())}function render(d){company.innerHTML=Object.entries(d.company).map(([k,v])=>`<div><span class="dot">●</span><b>${k}</b><div>${v}</div></div>`).join('');event.innerHTML=d.event?`<b>${d.event.vehicle}</b> · operação <b>${d.event.operation}</b> · event_id <span class="small">${d.event.event_id}</span>`:'Nenhuma ocorrência activa.';messages.innerHTML=d.messages.map(m=>`<div class="msg"><b>${m.from} → ${m.to}</b><br>${m.text}</div>`).join('');proposal.innerHTML=d.proposal?`<h3>ResolutionProposal</h3><div class="proposal">Problema: ${d.proposal.problem}\nSectores: ${d.proposal.sectors.join(', ')}\nAções: ${JSON.stringify(d.proposal.actions,null,2)}\nAprovação necessária: ${d.proposal.approval_required}\nContexto: ${JSON.stringify(d.proposal.context,null,2)}</div>`:'';decision.innerHTML=d.decision?`<p>Decision <b>${d.decision.decision_id}</b>: <span class="status">${d.decision.status}</span></p>`:'';result.innerHTML=d.result?`<div class="proposal">status: <b>${d.result.status}</b>\nexecution_id: ${d.result.execution_id}\nresult_id: ${d.result.result_id}\ndetails: ${JSON.stringify(d.result.details,null,2)}</div>`:'';interpretation.innerHTML=d.master_interpretation?`<h3>Interpretação da IA Mestre</h3><div class="proposal">${d.master_interpretation}</div>`:''}load();</script></body></html>'''
