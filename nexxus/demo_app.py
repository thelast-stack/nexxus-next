...
<style>
:root{font-family:system-ui,-apple-system,sans-serif;color:#17202a;background:#eef1f5;--accent:#111827}
*{box-sizing:border-box}
body{margin:0}
header{background:#111827;color:#fff;padding:22px 32px;display:flex;align-items:baseline;gap:12px;box-shadow:0 2px 8px rgba(0,0,0,.12)}
header strong{font-size:18px;letter-spacing:.02em}
header div{color:#9aa4b2;font-size:13px}
main{max-width:1180px;margin:24px auto;padding:0 20px 60px}
.card{background:#fff;border:1px solid #e2e6ec;border-radius:14px;padding:20px;margin-bottom:16px;box-shadow:0 1px 3px rgba(16,24,40,.04)}
.card h2{margin-top:0;font-size:16px;letter-spacing:.01em}
.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}
.metric{padding:14px;background:#f8fafc;border-radius:10px;border:1px solid #eef1f5}
.metric b{display:block;margin-bottom:5px;font-size:12px;text-transform:uppercase;letter-spacing:.03em;color:#667085}
.step{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px}
.step span{padding:7px 10px;border-radius:999px;background:#eef2f6;color:#667085;font-size:13px}
.step .active{background:#111827;color:#fff}
.msg{padding:10px 12px;border-left:3px solid #9aa4b2;margin:7px 0;background:#fafafa;border-radius:0 8px 8px 0}
.box{border:2px solid #e2e6ec;border-radius:12px;padding:16px;margin-top:12px}
.suggestion{border-color:#111827}
.decision{border-color:#667085}
.commercial{border-color:#315c8c}
button{padding:10px 14px;border:1px solid #c7ced8;border-radius:8px;background:#fff;cursor:pointer;margin:4px 4px 0 0;font-size:14px}
button:hover{background:#f4f6f8}
button.primary{background:#111827;color:#fff;border-color:#111827}
button.primary:hover{background:#1f2a3a}
input,select{padding:9px;border:1px solid #c7ced8;border-radius:7px;margin:4px;width:calc(100% - 8px);font-size:14px}
form{display:grid;grid-template-columns:1fr 1fr;gap:4px}
...
.muted{color:#667085}
.ok{font-weight:700}
.tabs{display:flex;gap:6px;margin:20px 0 4px;border-bottom:1px solid #d9dee7}
.tab-btn{padding:11px 18px;border:none;background:transparent;border-radius:10px 10px 0 0;cursor:pointer;font-size:14px;color:#667085;margin:0;position:relative;top:1px}
.tab-btn:hover{background:#f4f6f8}
.tab-btn.active{background:#fff;color:#111827;font-weight:600;border:1px solid #d9dee7;border-bottom:1px solid #fff}
.tab-panel{display:none}
.tab-panel.active{display:block}
@media(max-width:800px){.grid{grid-template-columns:1fr 1fr}form{grid-template-columns:1fr}}
@media(max-width:520px){.grid{grid-template-columns:1fr}.tabs{overflow-x:auto}.tab-btn{white-space:nowrap}}
</style></head>
...
<button class="tab-btn active" data-tab="comercial">Comercial</button>
<button class="tab-btn" data-tab="ocorrencia">Ocorrência &amp; IA Mestre</button>
<button class="tab-btn" data-tab="director">Director &amp; Execução</button>
</div>
<div class="tab-panel active" id="panel-comercial">
<div class="card commercial"><h2>Transportes — Comercial</h2><div id="commercial"></div></div>
</div>
<div class="tab-panel" id="panel-ocorrencia">
...
<div class="tab-panel" id="panel-director">
...
el("tabs").addEventListener("click",function(e){var b=e.target.closest(".tab-btn");if(!b)return;document.querySelectorAll(".tab-btn").forEach(function(x){x.classList.remove("active")});document.querySelectorAll(".tab-panel").forEach(function(x){x.classList.remove("active")});b.classList.add("active");el("panel-"+b.dataset.tab).classList.add("active")});
