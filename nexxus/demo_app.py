...
class DemoState:
    ...
    def activate_breakdown(self) -> None:
        ...
        self.suggestion_accepted = False; self.decision = None; self.request = None; self.result = None; self.master_interpretation = None
    def accept_suggestion(self) -> None:
        if not self.proposal: raise ValueError("a MasterAI proposal is required")
        self.suggestion_accepted = True
    def decide(self, status: str) -> None:
        if status not in {"approved", "rejected"} or not self.proposal or not self.suggestion_accepted: raise ValueError("the suggestion must be accepted before human decision")
    ...
    def execute_success(self) -> None:
        if not self.request or not self.decision or self.decision.status != "approved": raise ValueError("an approved decision is required")
        self.result = simulate_success(self.result); self.master_interpretation = MasterAI().interpret_execution_result(self.result)
    def execute_failure(self) -> None:
        if not self.request or not self.decision or self.decision.status != "approved": raise ValueError("an approved decision is required")
        self.result = simulate_failure(self.result); self.master_interpretation = MasterAI().interpret_execution_result(self.result)
...
HTML = '''<!doctype html><html lang="pt"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NEXXUS NEXT</title><style>body{font-family:system-ui;margin:0;background:#f4f5f7;color:#17202a}header{padding:24px 32px;background:#111827;color:white}main{max-width:1100px;margin:24px auto;padding:0 20px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.card{background:white;border:1px solid #ddd;border-radius:12px;padding:18px;margin-bottom:16px}.dot{font-size:22px}button{padding:10px 14px;border:1px solid #bbb;border-radius:8px;background:white;cursor:pointer;margin:4px}button.primary{background:#111827;color:white}.msg{padding:9px;border-left:3px solid #aaa;margin:6px 0}.proposal{white-space:pre-wrap;background:#f8fafc;padding:12px;border-radius:8px}.status{font-weight:700;text-transform:uppercase}.small{color:#667085;font-size:13px}.suggestion{border:2px solid #ddd;padding:14px;border-radius:10px}.decision{border:2px solid #ddd;padding:14px;border-radius:10px;margin-top:12px}.muted{color:#667085}@media(max-width:800px){.grid{grid-template-columns:1fr}}</style></head><body><header><strong>NEXXUS NEXT</strong><div>Director Console · protótipo demonstrável</div></header><main><div class="card"><h2>Estado da empresa</h2><div class="grid" id="company"></div></div><div class="card"><h2>Operação</h2><div id="event">Nenhuma ocorrência activa.</div><button class="primary" id="breakdown">Simular avaria do veículo</button></div><div class="card"><h2>IA Mestre</h2><div id="messages"></div><div id="proposal"></div><div id="interpretation"></div></div><div class="card"><h2>Decisão do Director</h2><div id="decision"></div></div><div class="card"><h2>Execução experimental</h2><div id="execution"></div></div></main><script>
(function(){"use strict";var el=function(id){return document.getElementById(id);};
async function request(path){var response=await fetch(path,{method:"POST"});var data=await response.json();if(!response.ok)throw new Error(data.error||"Erro no servidor");render(data);}
function text(v){return String(v==null?"":v);}
function makeButton(label,cls,fn){var b=document.createElement("button");b.textContent=label;if(cls)b.className=cls;b.addEventListener("click",fn);return b;}
function render(data){var c=data.company||{};el("company").innerHTML="<div><span class='dot'>●</span><b>Transportes</b><div>"+text(c.transportes)+"</div></div><div><span class='dot'>●</span><b>Manutenção</b><div>"+text(c.manutencao)+"</div></div><div><span class='dot'>●</span><b>Financeiro</b><div>"+text(c.financeiro)+"</div></div><div><b>Operações pendentes</b><div>"+text(c.pending_operations)+"</div></div>";
el("event").textContent=data.event?"Veículo "+text(data.event.vehicle)+" avariado · operação "+text(data.event.operation)+" · "+text(data.event.event_id):"Nenhuma ocorrência activa.";
el("messages").innerHTML=(data.messages||[]).map(function(m){return "<div class='msg'><b>"+text(m.from)+" → "+text(m.to)+"</b><br>"+text(m.text)+"</div>";}).join("");
var p=el("proposal");p.innerHTML="";if(data.proposal){var q=data.proposal;var box=document.createElement("div");box.className="suggestion";box.innerHTML="<h3>IA Mestre — Sugestão</h3><p>Problema: "+text(q.problem)+"</p><div class='proposal'>Sectores: "+(Array.isArray(q.sectors)?q.sectors.join(", "):text(q.sectors))+"\nAções: "+JSON.stringify(q.actions||[],null,2)+"\nAprovação necessária: "+text(q.approval_required)+"\nContexto: "+JSON.stringify(q.context||{},null,2)+"</div>";if(!data.suggestion_accepted)box.appendChild(makeButton("Aceitar sugestão","primary",function(){request("/api/accept-suggestion").catch(showError);}));else{var a=document.createElement("p");a.innerHTML="<b>Sugestão aceite.</b> Aguardando decisão do Director.";box.appendChild(a);}p.appendChild(box);}
var d=el("decision");d.innerHTML="";if(data.suggestion_accepted&&!data.decision){var db=document.createElement("div");db.className="decision";db.innerHTML="<h3>Decisão do Director</h3><p>A resolução proposta pela IA Mestre está pronta para autorização.</p>";db.appendChild(makeButton("Aprovar execução","primary",function(){request("/api/decision?status=approved").catch(showError);}));db.appendChild(makeButton("Rejeitar","",function(){request("/api/decision?status=rejected").catch(showError);}));d.appendChild(db);}else if(data.decision)d.innerHTML="<p>Decision <b>"+text(data.decision.decision_id)+"</b>: <span class='status'>"+text(data.decision.status)+"</span></p>";
var x=el("execution");x.innerHTML="";if(data.decision&&data.decision.status==="approved"&&!data.result){var eb=document.createElement("div");eb.className="decision";eb.innerHTML="<p>O Core autorizou a execução experimental.</p>";eb.appendChild(makeButton("Executar com sucesso","primary",function(){request("/api/execute").catch(showError);}));eb.appendChild(makeButton("Simular falha","",function(){request("/api/fail").catch(showError);}));x.appendChild(eb);}else if(data.result)x.innerHTML="<div class='proposal'>status: <b>"+text(data.result.status)+"</b>\nexecution_id: "+text(data.result.execution_id)+"\nresult_id: "+text(data.result.result_id)+"\ndetails: "+JSON.stringify(data.result.details||{},null,2)+"</div>";else if(data.decision&&data.decision.status==="rejected")x.innerHTML="<p class='muted'>Execução bloqueada pelo Core.</p>";else x.innerHTML="<p class='muted'>A execução só fica disponível após aprovação do Director.</p>";
el("interpretation").innerHTML=data.master_interpretation?"<h3>Interpretação da IA Mestre</h3><div class='proposal'>"+text(data.master_interpretation)+"</div>":"";}
function showError(e){console.error(e);alert(e.message||String(e));}
el("breakdown").addEventListener("click",function(){request("/api/breakdown").catch(showError);});fetch("/api/state").then(function(r){return r.json();}).then(render).catch(showError);
}());
</script></body></html>'''
