...
async function post(path,body){try{var o={method:"POST"};if(body!==undefined){o.headers={"Content-Type":"application/json"};o.body=JSON.stringify(body)}var r=await fetch(path,o),d=await r.json();if(!r.ok)throw new Error(d.error||"Erro no servidor");render(d)}catch(e){console.error(e);alert(e.message||String(e))}}
function button(parent,label,primary,path,body){var b=document.createElement("button");b.textContent=label;if(primary)b.className="primary";b.addEventListener("click",function(){post(path,body)});parent.appendChild(b)}
function render(d){
 var c=d.company||{};
 el("company").innerHTML="<div class='metric'><b>Transportes</b>"+t(c.transportes)+"</div><div class='metric'><b>Manutenção</b>"+t(c.manutencao)+"</div><div class='metric'><b>Financeiro</b>"+t(c.financeiro)+"</div><div class='metric'><b>IA Mestre</b>"+t(c.master)+"</div>";
 var tr=d.transport||{},p=el("commercial");p.innerHTML="";
 if(tr.selected_client){var cp=document.createElement("p");cp.innerHTML="<b>Cliente seleccionado:</b> "+t(tr.selected_client.name)+" — "+t(tr.selected_client.identification);p.appendChild(cp)}
 var cf=document.createElement("form");cf.innerHTML="<input name='name' placeholder='Cliente — nome' required><input name='contact' placeholder='Contacto' required><input class='full' name='identification' placeholder='NIF / identificação' required>";var cb=document.createElement("button");cb.className="primary full";cb.textContent="Criar / seleccionar cliente";cf.appendChild(cb);cf.addEventListener("submit",function(e){e.preventDefault();post("/api/transport/client",Object.fromEntries(new FormData(cf)))});p.appendChild(cf);
 if((tr.clients||[]).length){var s=document.createElement("select");s.innerHTML="<option value=''>Seleccionar cliente existente</option>"+(tr.clients||[]).map(function(x){return "<option value='"+t(x.client_id)+"'>"+t(x.name)+" — "+t(x.identification)+"</option>"}).join("");s.addEventListener("change",function(){if(this.value)post("/api/transport/client/select",{client_id:this.value})});p.appendChild(s)}
 var rf=document.createElement("form");rf.innerHTML="<input name='cargo_description' placeholder='Descrição da carga' required><input name='quantity' type='number' min='1' placeholder='Quantidade' required><input name='weight_kg' type='number' min='0' step='0.1' placeholder='Peso (kg)' required><input name='volume_m3' type='number' min='0' step='0.1' placeholder='Volume (m³)' required><input name='cargo_type' placeholder='Tipo de carga' required><input name='origin' placeholder='Origem / recolha' required><input name='destination' placeholder='Destino / entrega' required><input name='distance_km' type='number' min='0' step='1' placeholder='Distância km (mock opcional)'><input name='priority' placeholder='Prioridade' value='normal'><input class='full' name='observations' placeholder='Observações'>";var rb=document.createElement("button");rb.className="primary full";rb.textContent="Criar pedido de transporte";rf.appendChild(rb);rf.addEventListener("submit",function(e){e.preventDefault();post("/api/transport/request",Object.fromEntries(new FormData(rf)))});p.appendChild(rf);
 if(tr.request){var rq=document.createElement("div");rq.className="box";rq.innerHTML="<h3>Pedido "+t(tr.request.request_id)+"</h3><p>"+t(tr.request.origin)+" → "+t(tr.request.destination)+" · "+t(tr.request.cargo_description)+" · "+t(tr.request.weight_kg)+" kg</p>";button(rq,"Criar Proforma",true,"/api/transport/proforma",{request_id:tr.request.request_id});p.appendChild(rq)}
 if(tr.proforma){var pp=document.createElement("div");pp.className="box";pp.innerHTML="<h3>Proforma "+t(tr.proforma.proforma_id)+" — "+t(tr.proforma.status)+"</h3><p>Valor preliminar: <b>"+t(tr.proforma.total)+" EUR</b></p><p>Pedido: "+t(tr.proforma.request_id)+" · Cliente: "+t(tr.proforma.client_name)+"</p>";if(tr.proforma.status==="payment_pending")button(pp,"Simular confirmação de pagamento",true,"/api/transport/payment",{proforma_id:tr.proforma.proforma_id});pp.innerHTML+=tr.operation_eligible?"<p class='ok'>✓ Pedido elegível para futura Operação.</p>":"<p class='muted'>Operação bloqueada até confirmação do pagamento.</p>";p.appendChild(pp)}
 var stages=["Estado","Detecção","Análise Mestre","Sugestão","Decisão Director","Core / Execução","Resultado"],active=d.result?6:d.decision?5:d.suggestion_accepted?4:d.proposal?3:d.event?2:0;
 el("steps").innerHTML=stages.map(function(s,i){return "<span class='"+(i===active?"active":"")+"'>"+(i+1)+". "+s+"</span>"}).join("");
 el("event").textContent=d.event?"Veículo "+t(d.event.vehicle)+" avariado — operação "+t(d.event.operation)+" — "+t(d.event.event_id):"Nenhuma ocorrência activa.";
 el("messages").innerHTML=(d.messages||[]).map(function(m){return "<div class='msg'><b>"+t(m.from)+" → "+t(m.to)+"</b><br>"+t(m.text)+"</div>"}).join("");
 var pr=el("proposal");pr.innerHTML="";
 if(d.proposal){var b=document.createElement("div");b.className="box suggestion";b.innerHTML="<h3>IA Mestre — RECOMENDAÇÃO</h3><p>"+t(d.proposal.problem)+"</p>";var pre=document.createElement("pre");pre.textContent="Sectores: "+(Array.isArray(d.proposal.sectors)?d.proposal.sectors.join(", "):t(d.proposal.sectors))+"\nAcções: "+JSON.stringify(d.proposal.actions||[],null,2)+"\nAprovação necessária: "+t(d.proposal.approval_required);b.appendChild(pre);if(!d.suggestion_accepted&&!d.suggestion_rejected){button(b,"Aceitar sugestão",true,"/api/accept-suggestion");button(b,"Rejeitar sugestão",false,"/api/reject-suggestion")}else{var q=document.createElement("p");q.innerHTML=d.suggestion_rejected?"<b>Sugestão rejeitada pelo Director.</b> O fluxo não avança.":"<b>Sugestão aceite pelo Director.</b> Aguardando autorização da execução.";b.appendChild(q)}pr.appendChild(b)}
 var dec=el("decision");dec.innerHTML="";
 if(d.suggestion_accepted&&!d.decision){var db=document.createElement("div");db.className="box decision";db.innerHTML="<h3>DECISÃO NECESSÁRIA</h3><p>A aceitação da recomendação não autoriza execução. O Director deve decidir separadamente.</p>";button(db,"APROVAR EXECUÇÃO",true,"/api/decision?status=approved");button(db,"REJEITAR EXECUÇÃO",false,"/api/decision?status=rejected");dec.appendChild(db)}
 else if(d.decision){dec.innerHTML="<p>Decision <b>"+t(d.decision.decision_id)+"</b>: <span class='status'>"+t(d.decision.status)+"</span></p>"}
 else if(d.suggestion_rejected){dec.innerHTML="<p class='muted'>Recomendação rejeitada. Não existe autorização.</p>"}
 var ex=el("execution");ex.innerHTML="";
 if(d.decision&&d.decision.status==="approved"&&!d.result){var xb=document.createElement("div");xb.className="box";xb.innerHTML="<p><b>Core:</b> aprovação válida. Execução experimental disponível.</p>";button(xb,"Executar com sucesso",true,"/api/execute");button(xb,"Simular falha",false,"/api/fail");ex.appendChild(xb)}
 else if(d.result){var xr=document.createElement("pre");xr.textContent="status: "+t(d.result.status)+"\nexecution_id: "+t(d.result.execution_id)+"\nresult_id: "+t(d.result.result_id)+"\ndetails: "+JSON.stringify(d.result.details||{},null,2);ex.appendChild(xr)}
 else{ex.innerHTML="<p class='muted'>Execução disponível apenas após aprovação humana válida.</p>"}
